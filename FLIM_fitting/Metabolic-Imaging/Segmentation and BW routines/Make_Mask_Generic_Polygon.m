function Make_Mask_Generic_Polygon(acqpath,poss,thresh)
...
% Inputs:
% -poss: uManager position
% -thresh: within the polygons, keep the option of including only a
%          threshold

% clear all;
% acqpath = 'Z:\Lab\Tim\2016-06-27 200 Embryos\Fertilitech Primovision Dishes\s1_FAD_zscan_750nm_4x4dish\';
...
im = imread([tifacqpaths{1} Dtifs{1}(fr).name]);
th = figure; imshow(im,[]);
hp = impoly;
active_region = getPosition(hp);
polymask = poly2mask(active_region(:,1), active_region(:,2), size(im,2), size(im,1));
close(th)
