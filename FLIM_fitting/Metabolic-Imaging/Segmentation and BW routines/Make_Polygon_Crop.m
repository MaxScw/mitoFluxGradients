function Make_Polygon_Crop(acqpath,posnum,fr,thresh)
% Once redox side-by-side tiff intensity images have been made, use this
% routine to manually draw the mask as a polygon (or several).

% Inputs:
% -posnum: uManager position
% -fr: frame that you want to use to draw the manual crop
% -thresh: within the polygons, keep the option of including only a
%          threshold

% 2019-02-27: Adapt to just record the polygon coordinates for each
% position. Then other routines (Make_Masks_from_IlastikProb) can read
% those coords in if you want to crop on top of segmentation.


% clear all;
% acqpath = 'Z:\Lab\Marta\2018-03-26 Embryo viability in  Glass\2018-09-14 Developmental range glass dish with beam dump\s1_a8_all\';
% nClusts = 4; KeepClusts = [2 3];
% Gblur = 10;
% area_cuts = [100 10000]
% thresh = [0 10];
% posnum = 15;

if acqpath(end)~='\' acqpath = [acqpath '\']; end;
load([acqpath 'multiD_indices.mat']);
NADHBool = 1; FADBool = 1;
if isempty(cell2mat(strfind(nameinds(:,4),'NADH'))) NADHBool = 0; end
if isempty(cell2mat(strfind(nameinds(:,4),'FAD'))) FADBool = 0; end
if ~exist('thresh')|thresh==-1 thresh = [0 10^6 0 10^6]; end % Unless specified, brightest 2 clusters is pretty good at getting mitochondria and excluding nucleus
if ~exist('fr')|fr==-1 fr = 1; end
if ~exist('posnum')|posnum==-1 posnum = 1; end

ProcDisp = 0;
% close all;
G = fspecial('gaussian',[5 5],5);

% Find cal_files folder
if exist([acqpath 'cal_files'])==7 % if cal_files is in acqpath
    calpath = [acqpath 'cal_files\'];
elseif exist([UpOneDir(acqpath) 'cal_files'])==7 % if cal_files in daypath
    calpath  = [UpOneDir(acqpath) 'cal_files\'];
else
    error('Cannot locate cal_files folder. Place in daypath or acqpath, please');
end

% Load IllProfCal.mat file
ProfBool = 0;
Dprof = dir([calpath 'IllProfCal*.mat']);
if ~isempty(Dprof)
    load([calpath Dprof(1).name]);
    IllProfCal = double(IllProfCal);
    %         IllProfCaldual = [IllProfCal IllProfCal];
    ProfBool = 1;
end

slashes = strfind(acqpath,'\');
Run = acqpath(slashes(end-1)+1:end-1);
Dpos = dir([acqpath 'sorted_sdts\*Pos*']);
Dpos(~[Dpos.isdir]) = [];
remove = [];
for i = 1:length(Dpos)
    if length(Dpos(i).name)>5
        remove = [remove i];
    end
end

Dpos(remove)=[];
if ~isempty(Dpos)
    srtacqpath = [acqpath 'sorted_sdts\'];
    for i = 1:size(Dpos,1)
        FLIMageacqpaths{i} = [srtacqpath 'FLIMageTiffs_' Dpos(i).name '_' Run '\'];
        tifacqpaths{i} = [srtacqpath 'IntTiffs_' Dpos(i).name '_' Run '\'];
        Dtifs{i} = dir([tifacqpaths{i} '*.tif']);
%         DFLIMages{i} = dir([tifacqpaths{i} '*.tif']);
        % save figures with overlaid ROIs to do quick checks after batch processing
        ROIacqpaths{i} = [srtacqpath 'ROIsCheckGen_' Dpos(i).name '_' Run '\'];
%         [a,b] = mkdir(ROIacqpaths{i});
    end
else
    srtacqpath = acqpath;
end

im = imread([tifacqpaths{posnum} Dtifs{1}(fr).name]);
sz = size(im)
th = figure; imshow(im(1:sz(1),1:sz(1)),[0 10]);
hp = impoly;
active_region = getPosition(hp);
polymask = poly2mask(active_region(:,1), active_region(:,2), size(im,1), size(im,1));
close(th)

save([acqpath 'PolyCrop_Pos' num2str(posnum-1)],'polymask');

% 
% for posnum = 1:size(Dpos,1)
%     
%     uManPos = Dpos(posnum).name(4:end);
% %     strnums = sscanf(uManPos ,'%g'); %Find the numbers in the name
% %     uManPos = strnums(1); % Assume name starts with 'Pos#' and the first number is the pos number
%     PosInd = strcmp(nameinds(:,3),uManPos); PosInd = PosInd';
%     
%     % Maybe you only want to do certain positions, like if you want to redo
%     % certain positions with different image processing parameters
%     if exist('posnum')&posnum~=-1
%         if ~strcmp(num2str(posnum),uManPos)
%             continue;
%         end
%     end
%     
%     % Maybe you only want to do certain positions, like if you want to redo
%     % certain positions with different image processing parameters
%     if exist('posnum')&posnum~=-1
%         if ~strcmp(num2str(posnum),uManPos)
%             continue;
%         end
%     end
%     
%     frames = unique(sort([nameinds{PosInd&[nameinds{:,7}]>-1,6}]));
%     frames = frames(frames>0);
%     ts = unique(sort([nameinds{PosInd,2}]))+1;
%     Zs = unique(sort([nameinds{PosInd,5}]))+1;
%     
%     % +1 because uMan indexes start at 0, I like 1
%     if ~exist('rang') rang = [min(Zs) max(Zs)]; end;
%     
%     set(gcf,'PaperPositionMode', 'auto')
%     clear Nmasks Fmasks Masks EggVals NADim FADim
%     
%     
%     for i = 1:length(frames)
%         
%         t(i) = unique(sort([nameinds{PosInd&[nameinds{:,6}]==frames(i),3}]))+1;
%         Z(i) = unique(sort([nameinds{PosInd&[nameinds{:,6}]==frames(i),5}]))+1;
%         disp(['Pos' uManPos ', Fr ' num2str(frames(i))])
%         
%         im = double(imread([tifacqpaths{posnum} 'fr' num2str(frames(i),'%05i') '.tif']));
%         if ProfBool
%             im = im./IllProfCaldual(:,1:size(im,2)).*mean(IllProfCaldual(:));
%         end
% %         FLim = imread([FLIMageacqpaths{posnum} 'fr' num2str(frames(i),'%05i') '.tif']);
%         xdim = size(im,2); ydim = size(im,1);
%         
%         if NADHBool&FADBool
%             xdim1ch = xdim/2;
%             NADim = im(:,1:xdim1ch); %NADFLim = FLim(:,1:xdim1ch);
%             Ngim = imfilter(NADim,G,'same');
%             FADim = im(:,xdim1ch+1:end); %FADFLim = FLim(:,xdim1ch+1:end);
%             Fgim = imfilter(FADim,G,'same');
%             threshpix = [find(NADim<thresh(1)|NADim>thresh(2)); find(FADim<thresh(3)|FADim>thresh(4))];
%             NADim(threshpix) = 0; FADim(threshpix) = 0;
%             KMim = [NADim FADim]; %KMFLim = [NADFLim FADFLim];
%         elseif NADHBool&~FADBool
%             NADim = im; %NADFLim = FLim;
%             Ngim = imfilter(NADim,G,'same');
%             FADim = [];
%             threshpix = [find(NADim<thresh(1)|NADim>thresh(2))];
%             KMim = [NADim]; %KMFLim = [NADFLim];
%         elseif ~NADHBool&FADBool
%             NADim = [];
%             FADim = im; %FADFLim = FLim;
%             Fgim = imfilter(FADim,G,'same');
%             threshpix = [find(FADim<thresh(1)|FADim>thresh(2))];
%             KMim = [FADim]; %KMFLim = [FADFLim];
%         else
%             error('Something wrong with channels')
%         end
%         
% %         [masks singlemask numeggs(i)] = Masks_Kmeans_FLIMages(KMim,KMFLim,Gblur,nClusts,KeepClusts,area_cuts,-1);
%         singlemask = polymask;
%         singlemask(threshpix) = 0;
%         % Bad frames?
%         if isempty(find(singlemask))
%             Masks{frames(i)}(1).NL = []; Masks{frames(i)}(1).FL = [];
%             Masks{frames(i)}(1).NLper = []; Masks{frames(i)}(1).FLper = [];
%             imshow(im,[min(im(:)) max([min(im(:))+1,max(im(:))*.4])],'Border','tight'); hold on;
%             set(gcf,'PaperPositionMode', 'auto')
%             text(xdim/2,ydim/2,'No mask found','color','red','HorizontalAlignment','center','fontsize',25)
%             set(gcf,'position',[200 200 xdim*.6 ydim*.6]);
%             if ~ProcDisp
%                 saveas(imh,[ROIacqpaths{posnum} 'ROI_im' num2str(frames(i),'%03i') '.png'],'png');
%             end
%         else
%             imshow(im,[min(im(:)) max([min(im(:))+1,max(im(:))*.4])],'Border','tight'); hold on;
%             set(gcf,'PaperPositionMode', 'auto')
%             if NADHBool&FADBool
%                 [y,x] = find(bwperim(singlemask));
%                 plot(x,y,'b.','markersize',3);
%                 plot(x+xdim/2,y,'g.','markersize',3)
%             elseif NADHBool&~FADBool
%                 [y,x] = find(bwperim(singlemask));
%                 plot(x,y,'b.','markersize',3);
%             elseif ~NADHBool&FADBool
%                 [y,x] = find(bwperim(singlemask));
%                 plot(x,y,'g.','markersize',3);
%             end
%             %     set(gca,'position',[.52 .02 .45 .45]) % for multi-plot
%             hold off;
%             if ProcDisp
%                 pause(1)
%             else
%                 set(gcf,'position',[200 200 xdim*.6 ydim*.6]);
%                 saveas(imh,[ROIacqpaths{posnum} 'ROI_im' num2str(frames(i),'%03i') '.png'],'png');
%                 pause(.01)
%             end
%             Masks{frames(i)}(1).NL = singlemask; Masks{frames(i)}(1).FL = singlemask;
%             Masks{frames(i)}(1).NLper = bwperim(singlemask); Masks{frames(i)}(1).FLper = bwperim(singlemask);
%             pause(.1)
%         end
%     end
%     save([acqpath acqpath(slashes(end-1)+1:end-1) '_' Dpos(posnum).name '_PolyMask.mat'],'frames','Masks');
% end

