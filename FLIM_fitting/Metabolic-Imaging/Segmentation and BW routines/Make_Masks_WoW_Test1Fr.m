function Make_Masks_WoW_Test1Fr(path,poss,area_cuts,Gblur,nClusts,KeepClusts,thresh,circ_cut,NumWells,frames,TxtBool)
...
if ~exist('TxtBool')|TxtBool==-1 TxtBool = 1; end % Unless specified, assume display text over individual eggs
if NumWells==9
    WellRad = 210;
elseif NumWells==16
    WellRad = 140;
...
%     frames = frames(frames>0);
    ...
    if NADHBool&FADBool
        ...
        xdim1ch = size(im,2)/2; ydim = size(im,1);
        FADim = im(:,xdim1ch+1:end); 
        Gwell = fspecial('gaussian',[10 10],10);
        gim = uint8(imfilter(InvertIm(FADim),Gwell,'same'));
        if WellRad> 0 NADim = WoWCircCrop(NADim,Cx,Cy,WellRad); FADim = WoWCircCrop(FADim,Cx,Cy,WellRad); end
    end %Do other cases later if need be
...
if NADHBool&FADBool
        ...
        elseif NADHBool&~FADBool
            ...
            NADim(threshpix) = 0;
            if WellRad> 0 NADim = WoWCircCrop(NADim,Cx,Cy,WellRad); end
        ...
        elseif ~NADHBool&FADBool
            ...
            FADim(threshpix) = 0;
            if WellRad> 0 FADim = WoWCircCrop(FADim,Cx,Cy,WellRad); end
...
%             xdim1ch = xdim;
            ...
            if WellRad> 0 UserChim = WoWCircCrop(UserChim,Cx,Cy,WellRad); end
            KMim = [UserChim]; 
        else
        ...
        for eg = 1:size(Masks0{frames(i)},2)
            if NADHBool&FADBool
                ...
                if TxtBool text(mean(x),mean(y),num2str(eg),'HorizontalAlignment','center','fontsize',17,'color','r'); end
                [y,x] = find(Masks0{frames(i)}(eg).FLper);
                plot(x+xdim1ch,y,'g.','markersize',3)
                if TxtBool text(mean(x)+xdim1ch,mean(y),num2str(eg),'HorizontalAlignment','center','fontsize',17,'color','r'); end
            elseif NADHBool&~FADBool
                [y,x] = find(Masks0{frames(i)}(eg).NLper);
                plot(x,y,'b.','markersize',3)
                if TxtBool text(mean(x),mean(y),num2str(eg),'HorizontalAlignment','center','fontsize',17,'color','r'); end
