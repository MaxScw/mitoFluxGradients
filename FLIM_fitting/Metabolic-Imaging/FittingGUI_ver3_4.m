...
% Last Modified by GUIDE v2.5 04-Apr-2014 15:03:44

% Versions:
% 2018-06-08: Updated to handle multi-channel fits with 'cyto' and 'mito'
...
%    handles.decay_struct{i}.fit_result(1,3) = 1;
   ...
   handles.decay_struct{i} = orderfields(handles.decay_struct{i});
end
...
set(handles.MainGui,'CurrentAxes',handles.Residual_axes)
...
set(handles.MainGui,'CurrentAxes',handles.ResidualHist_axes)    
xlim([-5,5])
...
for i = 1:NumOfDecays
    decay_handle(i) = decay_struct{i}.decay_handle(1);
    fit_handle(i) = decay_struct{i}.fit_handle(1);
    residual_handle(i) = decay_struct{i}.residual_handle(1);
end
...
for i = 1:length(selected)
    ...
    elseif decay_struct{selected(i)}.fitting_method ~= 0
        ...
        residual_handle(selected(i)) = plot(time,residual,'Color','k');
        xlim([min(time) max(time)])
    end
...
set(handles.FitResult_table,'Data',decay_struct{selected(1)}.fit_result(:,:,1));
...
[filenames,pathname,filterindex] = uigetfile2('*.mat','MultiSelect','on');

if iscell(filenames)
    ind = 1:size(filenames,2);
else
    ind = 1;
    if filenames == 0
        return
...
for fs = ind
    ...
    loaded_decays = loaded(1).decays_fits_struct;
    loaded_decays(cellfun('isempty',loaded_decays))=[];
    try
        irf = loaded_decays{1}.irf;
    catch
        try
            irf = loaded_decays.irf;
        catch
            irf = loaded.irf
        end
    end
    ...
    handles.NumOfDecays = handles.NumOfDecays + length(loaded_decays);
    if length(loaded_decays)>=1
        set(handles.DeleteSelected_pushbutton,'Enable','on');
    end
    
    % Clear the plot handles and then replot them all
    for i = 1:length(loaded_decays)
        % Reset values to get rid of handles stored from previous analysis
        % session
        loaded_decays{i}.decay_handle = -99;
        loaded_decays{i}.fit_handle = -99;
        loaded_decays{i}.residual_handle = -99;
        
        
        % START PLOTTING
        time = loaded_decays{i}.time;
        dt = time(2)-time(1);
        L = length(loaded_decays{i}.decay);
        decay = loaded_decays{i}.decay(:,1);
        
        set(handles.MainGui,'CurrentAxes',handles.Decay_axes)
        fit_region = loaded_decays{i}.fit_region;
        noise_region = loaded_decays{i}.noise_region;
        % Decays (should always be there)
        loaded_decays{i}.decay_handle = semilogy(time,decay,'r.');
        % Fit Start/End lines
        fit_start_x = loaded_decays{i}.fit_region(1).*dt;
        fit_end_x = loaded_decays{i}.fit_region(2).*dt;
        noise_region_from = loaded_decays{i}.noise_region(1).*dt;
        noise_region_to = loaded_decays{i}.noise_region(2).*dt;
        if ishandle(handles.start_line)
            delete(handles.start_line);
            handles.start_line = line([fit_start_x;fit_start_x],[1;1E3],'Color','g');
        end
        if ishandle(handles.end_line)
            delete(handles.end_line);
            handles.end_line = line([fit_end_x;fit_end_x],[1;1E3],'Color','g');
        end
        if ishandle(handles.noise_line1)
            delete(handles.noise_line1);
            handles.noise_line1 = line([noise_region_from;noise_region_from],[1;1E3],'Color','y');
        end
        if ishandle(handles.noise_line2)
            delete(handles.noise_line2);
            handles.noise_line2 = line([noise_region_to;noise_region_to],[1;1E3],'Color','y');
        end
        % Fits (if they exist)
        if ~isempty(loaded_decays{i}.fit)
            fit = loaded_decays{i}.fit(:,1);
            loaded_decays{i}.fit_handle = semilogy(time(fit_region(1):fit_region(2)),fit,'Color','k','LineWidth',2);
        end
        % IRF:
        handles.irf = irf;
        handles.time = time;
        ...
        % Residuals
        if ~isempty(loaded_decays{i}.residual)
            set(handles.MainGui,'CurrentAxes',handles.Residual_axes)
            loaded_decays{i}.residual_handle = plot(time(fit_region(1):fit_region(2)),loaded_decays{i}.residual(fit_region(1):fit_region(2))); % Note, this is actually the weighted residual
            set(handles.MainGui,'CurrentAxes',handles.ResidualHist_axes)
            hist(loaded_decays{i}.residual,-5:5)
        end
    end
    
    handles.decay_struct = [handles.decay_struct; loaded_decays];
    
    % Load Fit Bounds used for this decay
    ...
    % Reset listbox
    for i = 1:handles.NumOfDecays
        names{i} = handles.decay_struct{i}.name;
    end
    set(handles.Decay_listbox,'String',names,'Value',1:handles.NumOfDecays)
end
guidata(hObject,handles);
% Decay_listbox_Callback(hObject, eventdata, handles)
...
for i = 1:length(selected)
    decay_handle(i) = decay_struct{selected(i)}.decay_handle;
    fit_handle(i) = decay_struct{selected(i)}.fit_handle;
    residual_handle(i) = decay_struct{selected(i)}.residual_handle;
    names{selected(i)} = [];
end
delete(findobj(decay_handle(ishandle(decay_handle))))
delete(findobj(fit_handle(ishandle(fit_handle))))
delete(findobj(residual_handle(ishandle(residual_handle))))

handles.NumOfDecays = NumOfDecays - length(selected);

set(handles.Decay_listbox,'String',names(~cellfun('isempty',names)),'Value',1:handles.NumOfDecays);

handles.decay_struct(selected) = [];

if isempty(handles.decay_struct)
    set(handles.DeleteSelected_pushbutton,'Enable','off')
end

% Refresh view if there are remaining decays
if handles.NumOfDecays>=1
    Decay_listbox_Callback(hObject, eventdata, handles)
end
...
try
    dt_irf = decay_to_fit{1}.IRFdt_ns;
    LaserT = decay_to_fit{1}.laserT_ns;
catch
    dt_irf = 0.0034186;
    LaserT = 12.659;
end
...
if fitting_method == 1   % Do LS fitting
    for j = selected
        ...
        try % Try to get the irf from the decay struct, but if it doesn't have one, take from handles
            irf = handles.decay_struct{j}.irf;
        catch
            irf = handles.irf;
        end
...
%         noise_region_to = round(str2num(get(handles.NoiseRegionto_edit,'String'))/dt);
        if decay_to_fit{j}.fit_region>0
            fit_start = decay_to_fit{j}.fit_region(1);
            fit_end = decay_to_fit{j}.fit_region(2);
            noise_region_from = decay_to_fit{j}.noise_region(1);
            noise_region_to = decay_to_fit{j}.noise_region(2);
        else
            dt = time(2)-time(1);
        fit_start = round(str2num(get(handles.FitStart_edit,'String'))/dt);
        fit_end = round(str2num(get(handles.FitEnd_edit,'String'))/dt);
...
% data to fit
        ...
        elseif nexpo == 2
            %         [shf,     A,    tau1,      f2,    tau2]
            p_init = [  0,     1,      .2,     0.3,       3]';
            p_min =  [-200,  0.7,    0.01,    0.01,    0.05]';
            p_max =  [200,     1,       1,       1,      10]';
        elseif nexpo == 3
            %         [ shf,    A,    tau1,      f2,    tau2,     f3,    tau3]
            p_init = [   0,    1,      .2,     0.3,     0.5,     .3,       3]';
            p_min =  [-200,  0.7,    0.05,    0.01,     1,   0.01,       1]';
            p_max =  [ 200,    1,      .1,       1,       5,      1,      10]';
        else
        ...
        [p_fit,Chi_sq,sigma_p,sigma_y,corr,R2,cvg_hst, converged] = ...
            lm(@lm_decay_model,p_init,time,decay,weight,dp,p_min,p_max,[nexpo,counts,fit_start,fit_end,dt_irf,LaserT],fit_start,fit_end,irf);
        toc
        ...
        y_hat = lm_decay_model(time,p_fit,[nexpo,counts,fit_start,fit_end,dt_irf,LaserT],irf);
...
%             p_min = [-26,0.96,2.1,0.15,0.01]';
%             p_max = [-20,1,3.2,0.35,0.3]';
            dp = [1,0.01,0.01,0.01,0.05]';
            p_min = [-26,0.96,2.1,0.15,0.01]';
            p_max = [-20,1,3.2,0.35,0.3]';
        ...
        y_hat = lm_decay_model(time,p_fit,[nexpo,counts,fit_start,fit_end],irf);
        y_hat = y_hat(fit_start:fit_end);
...
set(handles.FitResult_table,'Data',fit_result(:,:,1))
...
[filename,filepath,filterindex] = uiputfile2('*.mat');

file = [filepath filename];
if file == 0
    return;
end
decays_fits_struct = handles.decay_struct;
for i = 1:length(decays_fits_struct)
    decays_fits_struct{i} = orderfields(decays_fits_struct{i});
end
irf = handles.irf;
save(file,'decays_fits_struct','irf')
FLIMBatchParamPlot_v3f(file)
...
handles.time = time;
for i = 1:length(selected)
    handles.decay_struct{selected(i)}.irf = irf;
end
...
time = handles.time;
if length(irf)~=length(time)
    dt = mean(diff(time))./(length(irf)/length(time));
