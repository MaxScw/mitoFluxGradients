function MultiD_sdt_sort_Lab(acqpath)
...
% % % TEST in script mode:
% clear all; 
% acqpath = 'Z:\Lab\Marta\Mitotraker\2019-05-01 Mitotracker 1cell\2019-05-03 Day 3\Emb3_zscan_46HWP_20s\';

% Version:
% 2019-03-22: Updated to allow for analyzing data taken with MaiTai setup
% (that Xingbo set up).
...
Dsdt = dir([acqpath '*.sdt']);
sdtL = size(Dsdt,1);
[a,b] = mkdir(sdtpath);

% AcqSdtCorr:
% Was this a Labview-triggered acquisition? If not assume it was just a
% linear series of BH images and treat as a single-channel series. Auto-generate
% a 'AcqSdtCorr' array.
if exist([acqpath 'AcqSdtCorr.txt'])==2
    AcqSdtCorr = ReadTxtSprd2Cell([acqpath 'AcqSdtCorr.txt']);
else
    for i = 1:length(Dsdt)
        AcqSdtCorr{i,1} = i; % sdt number
        AcqSdtCorr{i,2} = 0; % Pos number
        AcqSdtCorr{i,3} = i-1; % Time frame
        AcqSdtCorr{i,4} = 'UserChan'; % Channel
        AcqSdtCorr{i,5} = 0; % z slice number
        AcqTRng1 = GetPhotonCollectTRange([acqpath Dsdt(i).name]);
%         AcqSdtCorr{i,6} = datestr(AcqTRng1(1),'yyyy-mm-dd,HH:MM:SS'); % sdt abs time stamp
        AcqSdtCorr{i,6} = AcqTRng1(1); % sdt abs time stamp
        AcqSdtCorr{i,7} = num2str(0); % z-readout from actuator (quality control)
    end
end

% Make an list, 'nameinds' that keeps track of the exact file correspondence
% [sdt# ,time point, position, channel, z-position, stack slice, scan start, scan end, num scans]
nameinds = {};

% AcqTRng is the start and stop times of the photon collection in
% datenum units (absolute time)
AcqTRng1 = GetPhotonCollectTRange([acqpath Dsdt(1).name]);
for i = 1:length(Dsdt)
    AcqTRng{i} = GetPhotonCollectTRange([acqpath Dsdt(i).name]);
...
for i = 1:min([size(Dsdt,1),size(AcqSdtCorr,1)])
    ...
    t = AcqSdtCorr{i,3};
    chan = AcqSdtCorr{i,4};
    % Quick convert from 'Custom' to 'UserChan' so it's compatible with subsequent code
    if strcmp(chan,'Custom') chan = 'UserChan'; end
    z = AcqSdtCorr{i,5};
    tstamps_Lab = datenum(AcqSdtCorr{i,6});
    Pos = num2str(AcqSdtCorr{i,2});
    
    % Using time stamps find the corresponding sdt file, which is
    % the file closest in time to trigger.
    diffs = tstamps_sdt-tstamps_Lab+tstampoffset;
    ...
    orig = [acqpath Dsdt(SdtMatchingInd).name];
    ...
    nameinds = [nameinds; {SdtMatchingInd,t,Pos,chan,z,-1,AcqTRng{SdtMatchingInd}(1),AcqTRng{SdtMatchingInd}(2),numscans}];
    
%     if ~isempty(dups)
%         diff1 = abs(tstamps_sdt(SdtMatchingInd)-nameinds{dups,8});
%         diff2 = abs(tstamps_sdt(SdtMatchingInd)-tstamps_Lab);
%         if diff1<diff2
