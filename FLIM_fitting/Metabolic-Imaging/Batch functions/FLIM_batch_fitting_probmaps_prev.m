function FLIM_batch_fitting_probmaps(acqpath,poss,MaskLab,Bin,fxshftBool,fxBgBool,IRFname,nexpo,TplotBool)
...
% Versions:
% 2019-04-11: Update to use FLIMAcqParamPlot instead of FLIMBatchParamPlot.
%  Make one time plot with multiple traces instead of a bunch of different
%  unweildy plots.
% 2018-08-11: Major rewrite - implement 3-exp fitting option
% 2018-08-02: Corrected TAC error in Tae's original 'lm_decay_model.m',
%  passing IRF acqpath to the model with 'consts' input.
% 2018-02-20: Adapted from FLIM_batch_fitting.m to analyze data obtained
% with Ilastik segmentation and probability maps. Now multiple decays are
% stored as extra dimensions of 'decays', 'fit_results', etc. Segment order
% is always 'mito','cyto','joint'
% 2018-01-18: incorporated new 'cal_files' organization
% 2015-05-29: Add binning capability.


% clear all;
% acqpath = 'C:\Users\Tim\Documents\Academic - Research\Publication Materials\2018 Intro to MetIm\Figures\Fig2\Blast_20180319\s1_a5';
% Bin = 1;
% fxshftBool=0;
% MaskLab='SingleMasks';

if acqpath(end)~='\' acqpath = [acqpath '\']; end
if ~exist('fxshftBool')|fxshftBool==-1 fxshftBool=1; end
...
if fxshftBool|fxBgBool load([UpOneDir(acqpath) 'ShiftsBG.mat']); end
fitting_method = 1;
...
if exist([acqpath 'cal_files'])==7 % if new cal_files folder exists
    ...
    CalPath = [UpOneDir(acqpath) 'DailyFiles\'];
end
...
if exist('IRFname')&IRFname~=-1
    ...
    for i = 1:length(Dirfsdt)
        ...
        % If IRF not named by wavelength, just load it in. We're only using
        % one these days, anyway
        if isempty(strfind(Dirfsdt(i).name,'750nm'))&isempty(strfind(Dirfsdt(i).name,'890nm'))&isempty(strfind(Dirfsdt(i).name,'845nm'))
            FIRFstruct = FLIMLoadIRF([CalPath Dirfsdt(i).name]);
            irftime = FIRFstruct.time;
        end
    end
        ...
        for j =  1:Bin:NumOfDecays 
            ...
            if fxshftBool
                ...
                elseif strfind(decay_struct{j}.name,'UserChan')
                    fit_result(1,1) = shifts(2);
                    %                     MaxIter = 10; % Junk fits, so why waste time
                else
                    error('Channel error')
                end
                fit_result(1,3) = 1;
...
if TplotBool FLIMAcqParamPlot(acqpath); end