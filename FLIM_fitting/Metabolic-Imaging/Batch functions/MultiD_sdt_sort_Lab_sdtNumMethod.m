function MultiD_sdt_sort_Lab_sdtNumMethod(path)
...
% clear all; path = 'Z:\Lab\Marta\2017-07-28 HCBI 1 photon\2017-08-04 HCBI Batch 1\s1_a1_allPos_postHCBI\';
...
for i = 1:min([size(Dsdt,1),size(AcqSdtCorr,1)])
    ...
    % Find sdt index
    ind1 = strfind(Dsdt(i).name,'sdt_c');
    ind2 = strfind(Dsdt(i).name,'.sdt');
    SdtInd = str2num(Dsdt(i).name((ind1+5):(ind2-1)));
    
    % Get metadata info for i-th trigger
    t = AcqSdtCorr{SdtInd,3};
    chan = AcqSdtCorr{SdtInd,4};
    z = AcqSdtCorr{SdtInd,5};
    tstamps_Lab(SdtInd) = datenum(AcqSdtCorr{SdtInd,6});
    Pos = num2str(AcqSdtCorr{SdtInd,2});
...
%     end
    ...
    nameinds = [nameinds; {SdtInd,Pos,t,chan,z,-1,AcqTRng{i}(1),AcqTRng{i}(2),numscans}];
