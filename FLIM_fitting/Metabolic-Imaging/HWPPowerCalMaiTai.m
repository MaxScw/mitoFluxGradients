function HWPPowerCalMaiTai(xlsfile)
...
% end
% clear all;
% xlsfile = 'C:\Users\Tim\Downloads\PowerCal_20X_FAD_SHGqwp_in.xlsx';

tab = xlsread(xlsfile);
angs = tab(tab(:,1)>0,1); pows = tab(tab(:,1)>0,2);
...
par0(1) = max(pows);
par0(2) = 48; % this can be optimized with center of mass.
par0(3) = 0; % Max extinction gives about 8mW at the objective

options = optimset('Display','off');   %changed from iter
par = lsqcurvefit(Isin,par0,angs,pows,[0 35 0],[10000 55 20],options);
x = min(angs)-20:.025:max(angs)+20;
...
spr = num2cell([outangs outpows]);
spr = [{'angle','output pow'};spr];
...
saveas(gcf,[xlsfile(1:dashes(end)-1) '_out.fig'])
