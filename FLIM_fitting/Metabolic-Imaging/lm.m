function [p,X2,sigma_p,sigma_y,corr,R_sq,cvg_hst, converged,y_hat] = lm(func,p,time,data,weight,dp,p_min,p_max,c,fit_start,fit_end,irf,MaxIter,DispBool)
...
if ~exist('DispBool') DispBool=1; end
...
[alpha,beta,X2,y_hat,dydp] = lm_matx(func,time,p,data,weight_sq,dp,c,fit_start,fit_end,irf);
...
while ( ~stop & iteration <= MaxIter )		% --- Main Loop
    ...
    y1 = feval(func,time,p_try,c,irf);
    ...
    if ( Update_Type == 2 )
        ...
        y2 = feval(func,time,p_try,c,irf);
...
function dydp = lm_dydp(func,time,p,y,dp,c,fit_start,fit_end,irf)
...
function [alpha,beta,Chi_sq,y_hat,dydp] = lm_matx(func,time,p,data,weight_sq,dp,c,fit_start,fit_end,irf)
...
y_hat = feval(func,time,p,c,irf);	% evaluate model using parameters 'p'
...
dydp = lm_dydp(func,time,p,y_hat,dp,c,fit_start,fit_end,irf);
