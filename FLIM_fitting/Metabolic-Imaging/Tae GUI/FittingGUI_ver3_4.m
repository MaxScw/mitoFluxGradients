...
% data to fit
        ...
        if nexpo == 1
            p_init = [0,1-est_noise/max(decay),1]';
            p_min = [-100,0.7,0.05]';
            p_max = [100,1,5]';
        elseif nexpo == 2
            p_init = [0,1-est_noise/max(decay),3,0.5,0.2]';
            p_min = [-200,0.7,0.05,0.01,0.01]';
            p_max = [200,1,5,1,0.9]';
        else
        ...
        [p_fit,Chi_sq,sigma_p,sigma_y,corr,R2,cvg_hst, converged] = ...
            lm(@lm_decay_model,p_init,time,decay,weight,dp,p_min,p_max,[nexpo,counts,fit_start,fit_end],fit_start,fit_end,irf);
        toc
