...
% Last Modified by GUIDE v2.5 11-Jun-2015 14:20:44
...
flim = double(handles.image_struct{selected}.flim);
...
if isempty(active_region)
    ...
    selected_pixel = zeros(size(flim,2),size(flim,3));
...
selected = get(handles.ImageSelection_slider,'Value');
img = handles.image_struct{selected}.image;
selected_pixel = boolean(handles.image_struct{selected}.selected_pixel);
selected_pixel_handle = handles.image_struct{selected}.selected_pixel_handle;
flim = double(handles.image_struct{selected}.flim);
decay = handles.image_struct{selected}.decay;
decay_handle = handles.image_struct{selected}.decay_handle;
...
mask = poly2mask(active_region(:,1), active_region(:,2), size(flim,2), size(flim,3));
...
selected_pixel_handle = plot(js,is,'rs','MarkerFaceColor','r','MarkerSize',5); %mark the point
...
set(handles.TotalCount_text,'String',num2str(sum(decay)))
%update total number of selected pixel
set(handles.NumOfSelectedPixel_text,'String',num2str(sum(sum(selected_pixel & mask))))
...
if isempty(current_active_region)==0
    ...
    for i = selected(selected~=current)
        ...
        flim = double(handles.image_struct{i}.flim);
...
for i = selected(selected~=current)
    ...
    flim = double(handles.image_struct{i}.flim);
    [is,js] = find(selected_pixel);
    decay = zeros(size(handles.image_struct{current}.decay));
    for j = 1:length(is)
        decay = decay+flim(:,is(j),js(j));
    end
    
    axes(handles.Image_axes);
    selected_pixel_handle = plot(js,is,'rs','MarkerFaceColor','r','MarkerSize',5);
    ...
    handles.image_struct{i}.decay = decay;
    handles.image_struct{i}.decay_handle = decay_handle;
end
guidata(hObject,handles);
...
% handles    structure with handles and user data (see GUIDATA)
selected = get(handles.FileName_listbox,'Value');
FileNames = get(handles.FileName_listbox,'String');
...
for i = selected
    filename = handles.image_struct{i}.filename;
    decay = handles.image_struct{i}.decay;
    decay_name = get(handles.DecayName_edit,'String');
    if isempty(decay_name)
        decay_name = FileNames{i};
    end
    img = handles.image_struct{i}.image;
    selected_pixel = handles.image_struct{i}.selected_pixel;
    dt = handles.image_struct{i}.dt;
    
    if length(selected)>1 & ~isempty(decay_name)
        names_to_save{ind} = [decay_name num2str(ind,'%04i')];
    else
        names_to_save{ind} = decay_name;
    end
    
    decay_to_save{ind}.decay = decay;
    decay_to_save{ind}.name = names_to_save{ind};
    decay_to_save{ind}.filename = filename;
    decay_to_save{ind}.image = img;
    decay_to_save{ind}.selected_pixel = selected_pixel;
    
    time = (1:length(decay))'*dt;
    decay_to_save{ind}.time = time;
    
    set(handles.MainGui,'CurrentAxes',handles.Decay_axes)
    decay_handle = semilogy(time,decay,'b.-');
    set(decay_handle,'Visible','off');
    
    decay_to_save{ind}.decay_handle = decay_handle;
...
ind = ind-1;
names_to_save = names_to_save';

if length(selected)>1
    decay_name = {[decay_name num2str(ind,'%04i')]};
end

if NumOfSavedDecays == 0;
    set(handles.SavedDecay_listbox,'String',names_to_save)
    %Enable pushbuttons
    Switch_pushbuttons(handles,'on','decay')
else
    names = get(handles.SavedDecay_listbox,'String');
    set(handles.SavedDecay_listbox,'String',[names;names_to_save])
end

NumOfSavedDecays = NumOfSavedDecays + ind;
...
if name ~= 0
    load([pathname name]);
    decay_struct(cellfun('isempty',decay_struct))=[];
...
% --- Executes on button press in SetPolygonRegion_pushbutton.
function SetPolygonRegion_pushbutton_Callback(hObject, eventdata, handles)
% hObject    handle to SetPolygonRegion_pushbutton (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
% Draw polygon to define active region
axes(handles.Image_axes);
hp = impoly;
...
active_region_handle = hp;

[is,js] = find(selected_pixel & mask);
decay = zeros(size(decay));
for ind = 1:length(is)
