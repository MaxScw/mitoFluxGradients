function TotalInts = IntensitymatrixTiffs16bit(path,WrVal,Ch2Bool)
...
if WrVal==1
    ...
    if Ch2Bool [a1,b1] = mkdir([path '\SHGTiffs']); end;
end

% Check for IRPowVsT.mat file. If that data was taken, improve accuracy
% by scaling by the average laser power during the collection
DIRpow = dir([path '\IRPowVsT.mat']);
IRBool = 0;
if ~isempty(DIRpow)
    load([path '\IRPowVsT.mat']);
    IRBool = 1;
end
...
%         asdf(i) = (AcqRng(2)-AcqRng(1))*86400;
        ...
        if Ch2Bool
            ch_2 = imread([path '\SHGTiffs\' Dsdt(i).name(1:end-4) '.tif']);
        ...
        if IRBool
            [IRfacts(i) IRval] = IRScaleFactor(InitIRs(1),IR,IRt,AcqRng);
            TotalInts(i) = TotalInts(i)/IRfacts(i);
        end
    end
end
...
for i = frs2do %ind:length(Dsdt)
    ...
    try
        ...
        if WrVal==1            
            ...
            % imwrite(uint16(round(ch_1./max(max(ch_1)).*255)),filepath1,'tiff','Compression','none');
            imwrite(uint16(ch_1),filepath1,'tiff','Compression','none');
        elseif WrVal==2
            StkCh1(i).Dsdt=uint16(ch_1);
        end
    catch
    ...
    if Ch2Bool
        try
            ...
            if WrVal==1                
                filepath2=[path '\SHGTiffs\' Dsdt(i).name(1:end-3) 'tif'];
                % imwrite(uint16(round(ch_2./max(max(ch_2)).*255)),filepath2,'tiff','Compression','none');
                imwrite(uint16(ch_2),filepath2,'tiff','Compression','none');
            elseif WrVal==2
                StkCh2(i).Dsdt=uint16(ch_2);
            end
            TotalInts(i) = sum(sum(ch_1))+sum(sum(ch_2))/numscans(i);
...
if isdir(path)
    ...
    H2.LineStyle = '-.';
edit     legend('Phots/numscans','Phots','Tot','location','best')
    set(gca,'FontSize',12)
