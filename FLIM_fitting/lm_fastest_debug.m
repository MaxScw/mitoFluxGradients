function [p,X2,sigma_p,sigma_y,corr,R_sq,cvg_hst, converged,y_hat] = lm_fastest_debug(func,p,time,data,weight,dp,p_min,p_max,c,fit_start,fit_end,irf,MaxIter,dt_irf,T,coarse)

% [p,X2,sigma_p,sigma_y,corr,R_sq,cvg_hst, converged,y_hat] = lm_fastest_debug(func,p,time,data,weight,dp,p_min,p_max,c,fit_start,fit_end,irf,MaxIter,dt_irf,T,coarse)
...
if nargin < 12, irf = -1; end % flag value
...
prnt = 1;		% >1 intermediate results; >2 plots
if ~exist('MaxIter')|MaxIter==-1 MaxIter = 40*Npar; end	% maximum number of iterations
...
[alpha,beta,X2,y_hat,dydp] = lm_matx(func,time,p,data,weight_sq,dp,c,fit_start,fit_end,irf,dt_irf,T,coarse);
...
while ( ~stop & iteration <= MaxIter )		% --- Main Loop
    ...
    y1 = feval(func,time,p_try,c,irf,dt_irf,T,coarse); %%%%%FUNCTION EVALUATION CHANGED%%%%%%%%%
    ...
    if ( Update_Type == 2 )
        ...
        y2 = feval(func,time,p_try,c,irf,dt_irf,T,coarse); %%%%%%%%%FUNCTION EVALUATION CHANGED%%%%%%%
...
function dydp = lm_dydp(func,time,p,y,dp,c,fit_start,fit_end,irf,dt_irf,T,coarse)
...
for j=1:n                       % loop over all parameters
    ...
    if del(j) ~= 0
        y1=feval(func,time,p,c,irf,dt_irf,T,coarse); %%%%%%%%%%%%FUNCTION EVALUATION CHANGED%%%%%%%%%%%
        y1=y1(fit_start:fit_end);
        func_calls = func_calls + 1;
        ...
        if (dp(j) < 0)		% backwards difference
            ...
            p(j) = ps(j) - del(j);
            y2 = feval(func,time,p,c,irf,dt_irf,T,coarse); %%%%%%%%%%%%%%%%FUNCTION EVALUATION CHANGED%%%%%%%%%
            y2 = y2(fit_start:fit_end);
...
function [alpha,beta,Chi_sq,y_hat,dydp] = lm_matx(func,time,p,data,weight_sq,dp,c,fit_start,fit_end,irf,dt_irf,T,coarse)
...
y_hat = feval(func,time,p,c,irf,dt_irf,T,coarse);	%%%%%%%%%%%%%FUNCTION EVALUATION CHANGED%%%%%%%%%%%%%%% evaluate model using parameters 'p'
...
dydp = lm_dydp(func,time,p,y_hat,dp,c,fit_start,fit_end,irf,dt_irf,T,coarse);
