...
for temp=1:numel(temps)
    ...
    bar(temps(temp)+offset*(-2), min_jox_dec, ...
         ...
         'DisplayName',"$c_\mathrm{out}\leq$"+string(mid_oxy)+"\,\,\,\,M", 'HandleVisibility',vis)
    errorbar(temps(temp)+offset*(-2), min_jox_dec, sigma_max_jox_dec, ...
         'Color', 'black', 'MarkerSize',15, 'LineWidth',1.5, ...
         'DisplayName',"$c_\mathrm{out}\leq$"+string(mid_oxy)+"\,\,\,\,M", 'HandleVisibility','off')
    ...
    bar(temps(temp), mid_jox_dec,...
         ...
         'DisplayName',string(mid_oxy)+"\,\,\,\,M "+"$\leq c_\mathrm{out}\leq$"+string(high_oxy)+"\,\,\,\,M", ...
    ...
    bar(temps(temp)+offset*(2), max_jox_dec, ...
     ...
     'DisplayName',string(high_oxy)+"\,\,\,\,M "+"$\leq c_\mathrm{out}$", 'HandleVisibility',vis)
    errorbar(temps(temp)+offset*(2), max_jox_dec, sigma_max_jox_dec, ...
         'Color', 'black', 'MarkerSize',15, 'LineWidth',1.5, ...
         'DisplayName',string(high_oxy)+"\,\,\,\,M "+"$\leq c_\mathrm{out}$", 'HandleVisibility','off')
...
export_fig(fig, string(plot_path)+'Jox_vs_temp.eps')
...
for temp=1:numel(temps)
    ...
    errorbar(kelvin_temps(temp)+offset*2, min_jox_dec, sigma_max_jox_dec, ...
         ...
         'DisplayName',"c*\approx"+string(low_oxy)+"$\mu$M", 'HandleVisibility',vis)
...
bar(temps(t)+offset, ring_averaged_jox_list_mid(t),...
         'FaceColor', cs2(midi, :), 'LineWidth',1.5, ...
         'DisplayName',string(mid_oxy)+"\,\,\,\,M "+"$\leq c_\mathrm{out}\leq$"+string(high_oxy)+"\,\,\,\,M", 'HandleVisibility',vis)
...
bar(temps(t)+offset*2, ring_averaged_jox_list_high(t),...
         'FaceColor', cs2(maxi, :), 'LineWidth',1.5, ...
         'HandleVisibility',vis, 'DisplayName',string(high_oxy)+"\,\,\,\,M "+"$\leq c_\mathrm{out}$")
...
export_fig(fig, string(plot_path)+'ringAverageJox_vs_temp.eps')
...
for ring=start_ring:10
    ...
    log_rate = log(vMax_temps);
    sigma_log_rate = sqrt((sigma_vMax_temps./vMax_temps).^2 );
    else
    ylabel('ln($K_{\mathrm{M}}/{[1\mathrm{M}]}$)', 'Interpreter','latex')
    ...
    'MarkerSize',15, 'LineWidth',1.5, 'Color',cs(ring, :), 'LineStyle','--')
   
    deltaG_bykb = [deltaG_bykb, -p(1)*100*8.3];

    subplot(1, 3, 2)
    xlabel('distance from cell center ($r$) [\,\,\,\,m]', 'Interpreter','latex')
    if km == true
    ylabel('$\Delta \mathrm{G}_{K_\mathrm{M}}$ [$\mathrm{J}/\mathrm{mol}$]', 'Interpreter','latex')
    else
    ...
    plot(1./kelvin_temps, collaps_val, 'o',...
    ...
    ylabel('$(-\mathrm{R}/\Delta \mathrm{G}_{K_\mathrm{M}}) \cdot$ ln($K_{\mathrm{M}}/{[1\mathrm{M}]}$)', 'Interpreter','latex')
...
cb.TickLabels = round(dist(start_ring:end),2);
...
export_fig(fig2, string(plot_path)+'ringWiseKmReactionRate_vs_temp.eps')
else
...
for temp=1:numel(temps)
    ...
    if rescaled_params==true
        ...
        ylabel('$v_{\mathrm{max}}/(v_{\mathrm{max}}(\mathrm{R}) \mathrm{T})$ [\,\,\,\,M/s]', 'Interpreter','latex')
    else
...
if rescaled_params==true
    ...
    export_fig(fig, string(plot_path)+'vMax_vs_dist_temp.eps')
end
